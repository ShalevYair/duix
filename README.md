...
> קובץ HTML יחיד שהופך כל אתר חדשות לחוויית קריאה מותאמת אישית —
> ללא שרת, ללא framework, ללא התקנה.

---

## תיאור הפרויקט

DUIX (ראשי תיבות: **D**ynamic **UI**/**UX**) הוא POC לקורא חדשות שמבוסס על קובץ HTML סטטי אחד בלבד.
...
DUIX פועל ישירות מהדפדפן, מחלץ תוכן מכל אתר חדשות שהמשתמש מגדיר, ומציג אותו בתצוגה נקייה.
...
כל אתר נראה אחרת — חלק מעדיפים כרטיסיות, אחרים רשימות, מישהו רוצה את כותרות בלבד.
...
```
אתר חדשות → חילוץ מובנה → 4 מצבי תצוגה → בחירת משתמש
```

המשתמש מגדיר URL פעם אחת ובוחר את אופן הצגת החדשות שלו. המערכת מסנכרנת עם האתר בכל טעינה.
...
2. בדיקת localStorage
   ├─ יש העדפה שמורה → דלג לשלב 5
   └─ אין → הצג מסך Onboarding → המשתמש בוחר סגנון → שמור
       ↓
3. קריאת URL (ברירת מחדל: ynet, walla, mako בסדר עדיפויות)
...
│   ├── header.css      header, url-bar, loading, error, #main-content
...
│   ├── parser.js       parseGeneric + 6 strategies + parseRSS + showReport + utils
│   ├── views.js        renderAll, renderListView, renderCardsView, renderMagazineView, renderTickerView
│   ├── onboarding.js   SAMPLE, mini* generators, buildOnboarding, showOnboarding, selectView
│   └── app.js          state, SOURCES, PROXIES, loadNews, switchView, esc, firstSentence, INIT
├── README.md
├── .gitignore
└── LICENSE
```
...
אין React, אין npm, אין build process — `index.html` עובד בלחיצה כפולה (עם HTTP server מקומי לטעינת קבצים).
...
**מקורות ברירת מחדל (ללא URL ידני):**

| מקור | RSS | HTML fallback |
|------|-----|--------------|
| ynet.co.il | `StoryRss1854.xml` | `/` |
| walla.co.il | `rss.walla.co.il/feed/1` | `/` |
| mako.co.il | RSS ייעודי | `/` |
...
פעיל רק כאשר `document.activeElement` אינו `<input>`.
...
- HTTP server מקומי לטעינת קבצי CSS/JS (למשל: `npx serve .` או VS Code Live Server)
- **אין** Node.js, **אין** שרת, **אין** build

---

*נבנה כ-POC ל-Dynamic UI/UX — zero-dependency, modular structure.*
